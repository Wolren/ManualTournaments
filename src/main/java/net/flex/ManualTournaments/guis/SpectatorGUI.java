...
public class SpectatorGUI {
    ...
    public static void teleportationGUI(Player sender) {
        ...
        Bukkit.getOnlinePlayers().forEach(player -> {
            if (sender != player && FightFactory.isPlayerInAnyFight(player.getUniqueId()) && !spectators.contains(player.getUniqueId())) spectatorMenu.addButton(teleportButton(sender, player));
        });
